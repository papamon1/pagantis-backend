...
exports.createTransfer = function (req, res) {
  ...
  WalletCtrl.checkWallet(transferData.walletFrom)
    .then((docSource) => {
      if (docSource.length === 0) {
        return res.status(422).send({
          message:
            "An error occurred creating the transfer. Source account does not exist",
        });
      } else {
        WalletCtrl.checkWallet(transferData.walletTo)
          .then((docDestination) => {
            if (docDestination.length === 0) {
              return res.status(422).send({
                message:
                  "An error occurred creating the transfer. Destination account does not exist",
              });
            } else {
              WalletCtrl.updateWallet(
                transferData.walletFrom,
                      ...
                      .then((doc, err) => {
                        transfer.save((errors, createdTransfer) => {
                          if (errors) {
                            ...
                            return res.status(422).send({
                              message:
                                "An error occurred creating the transfer",
                              details: errors,
                            });
                          }
                          if (
                            transferData.walletTo === "60539b104a30d12faa811d7c"
                          )
                            Utils.handleResults(
                              null,
                              createdTransfer,
